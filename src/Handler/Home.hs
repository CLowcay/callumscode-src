...
getHomeR = do
  ...
  mblog <- runDB$ selectFirst
    [BlogPostDeleted ==. False] [Desc BlogPostTimeCreated, LimitTo 1]
  case mblog of
    Just (Entity _ blog) -> redirect$ BlogR
      (blogPostYear blog) (blogPostMonth blog) (blogPostUrl blog)
    Nothing -> defaultLayout $ do
      setTitle "Callum's Code"
      let {
        content = preEscapedToMarkup
